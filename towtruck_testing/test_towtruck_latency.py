...
class TestTowtruckLatency(Node):
	def __init__(self):
		...
		self.declare_parameter("main_robot_ip",'192.168.1.29')
	...
	def update_score(self, ip, port, endpoint, data):
		try:
			...
			response = requests.post(url, params=params, headers=headers, json=data, timeout=1)
	...
	def main(self):
		try:
			...
			if self.current_time - self.received_kinematic_time < 1.0:
				...
				return
			latency = ping(self.main_server_ip, timeout=1)
			if latency is None:
