...
from towtruck_testing.unit_edges import customer_uni_edges, DijsktraCalc
# from unit_edges import customer_uni_edges
# from unit_edges import uni_edges, customer_uni_edges
...
class PathWithLaneIDSubscriber(Node):
	def __init__(self):
		...
		self.declare_parameter("save_picture_path", "/home/nontanan/ros2_ws/src/towtruck_testing/resource/trajectory/")
		self.declare_parameter("update_path", False)
		...
		self.update_path = self.get_parameter("update_path").get_parameter_value().bool_value
		self.save_picture_path = os.path.join(
			self.get_parameter("save_picture_path").get_parameter_value().string_value,
		...
		self.route_num = 0
		self.save = False
		dji_cals = DijsktraCalc(main_server_ip="192.168.1.114",main_server_port="5010",reload=False)
		self.edges = dji_cals.edges
		# self.edges = customer_uni_edges
		
	def generate_paths_from_edges(self, edges):
		if self.route_num < len(edges):
			print(f"[LOOP] route_num: {self.route_num}/{len(edges)} | state: {self.route_state}")
	...
	def main(self):
		self.generate_paths_from_edges(self.edges)
		# self.merge_paths()
	...
	def segment_all_path(self):
		csv_directory = self.save_picture_path
		...
		for file in csv_files:
			...
			for idx in range(1, len(new_path['x'])):
				dist = math.sqrt(math.pow(new_path['x'][idx] - new_path['x'][idx - 1], 2) + math.pow(new_path['y'][idx] - new_path['y'][idx - 1], 2))
				distance.append(dist)
			print(f"len x: {len(new_path['x'])}, len dis: {len(distance)}")
			df = pd.DataFrame({'x': [float(v) for v in new_path['x']], 'y': [float(v) for v in new_path['y']], 'yaw': [float(v) for v in new_path['yaw']], 'distance': distance})
