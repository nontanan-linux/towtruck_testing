...
from towtruck_msgs.srv import RecordGoal, RecordHorn
...
class TowtruckRecordPoint(Node):
	def __init__(self):
		...
		self.horn_save_file = os.path.join(self.get_parameter("save_csv_path").get_parameter_value().string_value, f'HornPoints_{datetime.datetime.now()}.csv')
		self.create_station_service = self.create_service(RecordGoal, 'towtruck2/goalpoint/create', self.create_goalpose)
		self.create_horn_service = self.create_service(RecordHorn, 'towtruck2/hornpoint/cretae', self.create_hornpose)
		...
		self.goalpoints_save_service = self.create_service(Trigger, 'towtruck2/goalpoint/save', self.save_goalpoints_to_csv)
		self.hornpoints_save_service = self.create_service(Trigger, 'towtruck2/hornpoint/save', self.save_hornpoints_to_csv)
	...
	def euler_from_quaternion(self, quaternion):
		...
		yaw = atan2(siny_cosp, cosy_cosp)*(180.0 / math.pi) # convert radius to degree
		return roll, pitch, yaw
	
	def create_hornpose(self, request, response):
		try:
			horn_pose = {
			...
			response.message = f"Horn point station: {horn_pose['station']} node: {horn_pose['node']} at (({horn_pose['x']:.2f}, {horn_pose['y']:.2f}) recorded successfully.)"
			self.get_logger().info(f"Horn point station: {horn_pose['station']} node: {horn_pose['node']} at (({horn_pose['x']:.2f}, {horn_pose['y']:.2f}) recorded successfully.)")
		except Exception as recorn_horn_err:
			response.success = False
			response.message = f'Create Horn point error: {recorn_horn_err}'
		return response
	
	def create_goalpose(self, request, response):
		try:
			goal = {
			...
			self.get_logger().info(f"Node '{goal['name']}' at ({goal['x']:.2f}, {goal['y']:.2f}) recorded successfully.")
	...
	def save_goalpoints_to_csv(self, request, response):
		try:
			...
			if not os.path.exists(self.station_save_file):
				...
				msg = f"Goalpoints saved to new CSV file: {self.station_save_file}"
			else:
				df.to_csv(self.station_save_file, mode='a', header=False, index=False)
				msg = f"Goalpoints appended to existing CSV: {self.station_save_file}"
			self.get_logger().info(msg)
			response.success = True
			response.message = msg
		except Exception as e:
			error_msg = f"Failed to save goalpoints to CSV: {e}"
			self.get_logger().error(error_msg)
			response.success = False
			response.message = error_msg
		return response

	def save_hornpoints_to_csv(self, request, response):
		try:
			csv_dir = os.path.dirname(self.horn_save_file)
			os.makedirs(csv_dir, exist_ok=True)
			df = pd.DataFrame(self.hornpoints)
			if not os.path.exists(self.horn_save_file):
				df.to_csv(self.horn_save_file, index=False)
				msg = f"Goalpoints saved to new CSV file: {self.horn_save_file}"
			else:
				df.to_csv(self.horn_save_file, mode='a', header=False, index=False)
				msg = f"Goalpoints appended to existing CSV: {self.horn_save_file}"
			self.get_logger().info(msg)
			response.success = True
			response.message = msg
		except Exception as e:
